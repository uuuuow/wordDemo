...
CFixedCam2D::CFixedCam2D(VTCalibration mode, QObject *parent)
	: CPointsManage(mode, parent)
{
}

CFixedCam2D::~CFixedCam2D()
{
}

bool CFixedCam2D::fixdFirstFrame()
{
	...
	if (availableImagePoint(g_proInfo.frameInfos[headIndex].imagePoints[0]) &&
		...
		availableImagePoint(g_proInfo.frameInfos[headIndex].imagePoints[4])
		)
		return true;
	return false;
}

bool CFixedCam2D::imagePointsReady(int index)
{
...
bool CFixedCam2D::actualPointsReady()
{
...
void CFixedCam2D::addPoints(int index, QPoint point)
{
	...
	if (!fixdFirstFrame()) {
		...
		else if (INVALID_VALUE_INT == g_proInfo.frameInfos[index].imagePoints[4].x()) {
...
void CFixedCam2D::calcPointP0()
{
	...
	if (!fixdFirstFrame())
		return;
	for (int n = headIndex; n < g_proInfo.frameInfos.size(); ++n) {
		if (!imagePointsReady(n))
			continue;
		
		vector<cv::Point2f> corners(4);
		corners[0] = cv::Point2f(g_proInfo.frameInfos[headIndex].imagePoints[1].x(), g_proInfo.frameInfos[headIndex].imagePoints[1].y());
		corners[1] = cv::Point2f(g_proInfo.frameInfos[headIndex].imagePoints[2].x(), g_proInfo.frameInfos[headIndex].imagePoints[2].y());
		corners[2] = cv::Point2f(g_proInfo.frameInfos[headIndex].imagePoints[3].x(), g_proInfo.frameInfos[headIndex].imagePoints[3].y());
		corners[3] = cv::Point2f(g_proInfo.frameInfos[headIndex].imagePoints[4].x(), g_proInfo.frameInfos[headIndex].imagePoints[4].y());
...
void CFixedCam2D::notifyVelocity()
{
	emit sigNotifyVelocity();
}

void CFixedCam2D::initForecast() {
	cv::Mat firstFrame;
	if (!fixdFirstFrame())
		return;
	m_keyTrack.release();
...
void CFixedCam2D::doForecast(int index)
{
