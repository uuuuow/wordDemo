...
class CCameraCalibration : public QDialog
{
	Q_OBJECT

public:
	CCameraCalibration(QWidget *parent = Q_NULLPTR);
	~CCameraCalibration();
	static void readSettings(const cv::FileNode& node, CSettings& x, const CSettings& default_value = CSettings());
	...
	static bool runCalibration(CSettings& s, cv::Size& imageSize, cv::Mat& cameraMatrix, cv::Mat& distCoeffs,
		std::vector<std::vector<cv::Point2f> > imagePoints, std::vector<cv::Mat>& rvecs, std::vector<cv::Mat>& tvecs,
		std::vector<float>& reprojErrs, double& totalAvgErr);

	static void saveCameraParams(CSettings& s, cv::Size& imageSize, cv::Mat& cameraMatrix, cv::Mat& distCoeffs,
		const std::vector<cv::Mat>& rvecs, const std::vector<cv::Mat>& tvecs,
		const std::vector<float>& reprojErrs, const std::vector<std::vector<cv::Point2f> >& imagePoints,
		double totalAvgErr);

	bool runCalibrationAndSave(CSettings& s, cv::Size imageSize, cv::Mat&  cameraMatrix, cv::Mat& distCoeffs,
